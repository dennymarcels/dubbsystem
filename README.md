...
- XTTS currently needs `transformers` in the `4.52.1`–`4.55.x` range; this project pins `transformers>=4.52.1,<4.56` and `coqui-tts>=0.27.2,<0.27.3` because XTTS's bundled tortoise/GPT layer imports `transformers.pytorch_utils.isin_mps_friendly`, which was removed in later `transformers` releases (including the 5.x line). `coqui-tts` versions newer than `0.27.2` relax their own upper bound on `transformers` without that internal code path having been fixed, so installing the latest of both packages together breaks XTTS synthesis with an `ImportError`.
